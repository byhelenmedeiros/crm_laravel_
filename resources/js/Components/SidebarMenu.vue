<template>
  <div v-if="authStore.user" class="text-xs text-red-500 mb-2">
    Role ID: {{ authStore.user.role_id }}
  </div>
      ...
      <li v-for="(item, index) in menuItems" :key="index" class="flex flex-col">
        ...
        <template v-else>
          <div class="flex items-center px-4 py-2 hover:bg-gray-200 cursor-pointer">
            ...
            <button v-if="authStore.isSuperadmin && sidebarOpen" @click.stop="toggleSubmenu" class="ml-auto">
              <FontAwesomeIcon :icon="submenuOpen ? faChevronDown : faChevronRight" class="w-4 h-4" />
            </button>
          </div>
          <ul v-if="authStore.isSuperadmin" v-show="submenuOpen && sidebarOpen" class="ml-8">
